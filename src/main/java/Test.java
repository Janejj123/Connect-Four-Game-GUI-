package hw4;

import java.util.Scanner;

public class Test {
    public static void main(String[] args) {
        System.out.println("enter 1 for GUI, 2 for two AI's, 3 for user against AI");
        Scanner reader = new Scanner (System.in);
        int gameMode = reader.nextInt();

        if (gameMode==1) {
            new GUICF(new WenjunAI());
        } else if (gameMode==2) {
            CFPlayer ai1 = new WenjunAI();
            CFPlayer ai2 = new RandomAI();
            int n = 10;
            int winCount = 0;
            for (int i =0; i < n ; i++) {
                ConsoleCF game = new ConsoleCF(ai1, ai2);
                game.playOut();
                if(game.getWinner() == ai1.getName())
                    winCount++;
            }
            System.out.println(((double) winCount)/n);
        } else {
            ConsoleCF game = new ConsoleCF(new WenjunAI());
            game.playOut();
            System.out.println(game.getWinner() + " has won.");
        }
    }
}